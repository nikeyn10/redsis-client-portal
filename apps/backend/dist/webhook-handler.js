...
async function handler(req) {
    try {
        ...
        const payload = (0, monday_sdk_1.parseWebhookPayload)(JSON.parse(req.body));
        // Handle webhook challenge (initial setup)
        const challengeResponse = (0, monday_sdk_1.handleWebhookChallenge)(payload);
        ...
        const { event } = payload;
        // Extract event data
        const eventData = (0, monday_sdk_1.extractEventData)(event);
        ...
        if (!(0, monday_sdk_1.shouldNotifyClient)(event.type, columnId)) {
            return { statusCode: 200, body: JSON.stringify({ success: true }) };
        }
        ...
        const clientIdColumn = item.column_values.find((col) => col.id === 'client_id' || col.id === 'person');
        ...
        const emailColumn = client.column_values.find((col) => col.id === 'email' || col.type === 'email');
        ...
        const notificationType = (0, monday_sdk_1.getNotificationTypeFromEvent)(event);
        if (!notificationType) {
            return { statusCode: 200, body: JSON.stringify({ success: true }) };
        }
 ...
 */
async function sendNotification(notification, apiToken) {
