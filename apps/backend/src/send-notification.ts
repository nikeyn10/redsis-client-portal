...
import type { ApiResponse, NotificationPayload, EmailNotification } from '@portal/types';
 ...
 */
function buildEmailTemplate(notification: NotificationPayload): EmailNotification {
 ...
 * Main handler
 */
export async function handler(req: any): Promise<ApiResponse> {
  try {
    const notification: NotificationPayload = JSON.parse(req.body);
